...
def filter_universe(df: pd.DataFrame, index_filter_parameters: dict) -> pd.DataFrame:
    """Filters the universe of stock data by conditions on attributes."""
    return df[(df['dividendYield'] >= index_filter_parameters['dividend_yield_min']) &
              (df['marketCap'] >= index_filter_parameters['market_cap_min']) &
              (df['forwardPE'] <= index_filter_parameters['pe_ratio_max']) &
...
def get_sector_table(df: pd.DataFrame, filtered_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df[(df['sector'] != sector_target_parameters['sector_exclude_1']) & (
        df['sector'] != sector_target_parameters['sector_exclude_2'])]
...
def get_target_allocation(df: pd.DataFrame) -> pd.DataFrame:
    df = df.assign(
        target_allocation=df['position_weight'] * user_parameters['pers_index_allocation'])
    return df
...
def orders_to_csv(df: pd.DataFrame) -> None:
    timestr = time.strftime("%Y%m%d-%H%M%S")
    df.to_csv(f"personalized_index_{timestr}.csv", index=True)
    df = df[['symbol', 'order_quantity']]
    df['order_quantity'] = df['order_quantity'].apply(np.int64)
    df.to_csv(f"orders_{timestr}.csv", index=False)
    return None
...
if __name__ == '__main__':
    ...
    filtered_df = filter_universe(df, index_filter_parameters)
    sorted_df = sort_universe(filtered_df, rank_order)
    sector_table = get_sector_table(df, filtered_df)
    stock_selection = get_stock_selection(sorted_df, sector_table)
    personalized_index = get_stock_weights(stock_selection)
    personalized_index = get_target_allocation(personalized_index)
    ...
    orders_to_csv(personalized_index)
