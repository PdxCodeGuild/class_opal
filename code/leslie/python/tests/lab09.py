...
def get_counts():
    ...
    with open('christmas_carol.txt', 'r', encoding='utf-8') as file:
        ...
        # reads content and replaces spaces with NO spaces to get accurate character count
...
def find_ari(char_count, word_count, sentence_count):
    ari_num = math.ceil(((char_count/word_count) * 4.71) +
                        ((word_count/sentence_count) * 0.5) - 21.43)
    ...
    """)
    return [ari_num, grade_level, age]


find_ari(get_counts()[0], get_counts()[1], get_counts()[2])
